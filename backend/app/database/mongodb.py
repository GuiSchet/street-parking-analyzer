...
class DatabaseService:
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            await self.db.parking_spaces.create_index(
            ...
            # parking_events
            await self.db.parking_events.create_index(
                [("space_id", ASCENDING), ("timestamp", DESCENDING)]
            )
            await self.db.parking_events.create_index(
            ...
            # vehicle_detections
            await self.db.vehicle_detections.create_index(
                [("vehicle_id", ASCENDING), ("camera_id", ASCENDING)]
            )
            await self.db.vehicle_detections.create_index(
                [("camera_id", ASCENDING), ("active", ASCENDING)]
            )
            await self.db.vehicle_detections.create_index([("last_seen", DESCENDING)])
            await self.db.vehicle_detections.create_index(
                [("last_seen", ASCENDING)], expireAfterSeconds=86400
            )  # TTL: 24 horas

            # analytics
            await self.db.analytics.create_index(
                [("camera_id", ASCENDING), ("date", DESCENDING), ("hour", DESCENDING)], unique=True
            )
            ...
            await self.db.system_logs.create_index(
                [("timestamp", ASCENDING)], expireAfterSeconds=2592000
            )  # TTL: 30 días
    ...
    async def get_active_config(self, camera_id: str) -> Optional[Dict]:
        ...
        return await self.db.configurations.find_one({"camera_id": camera_id, "active": True})
    ...
    async def save_config(self, config: Dict) -> str:
        ...
        result = await self.db.configurations.update_one(
            {"camera_id": config["camera_id"]}, {"$set": config}, upsert=True
        )
    ...
    async def upsert_parking_space(self, space: Dict) -> str:
        ...
        result = await self.db.parking_spaces.update_one(
            {"space_id": space["space_id"]}, {"$set": space}, upsert=True
        )
    ...
    async def update_space_status(self, space_id: str, new_status: str, confidence: float) -> bool:
    ...
    async def get_space_history(self, space_id: str, hours: int = 24) -> List[Dict]:
        ...
        cursor = self.db.parking_events.find(
            {"space_id": space_id, "timestamp": {"$gte": since}}
    ...
    async def upsert_vehicle_detection(self, detection: Dict) -> str:
        ...
        result = await self.db.vehicle_detections.update_one(
            {"vehicle_id": detection["vehicle_id"], "camera_id": detection["camera_id"]},
            {"$set": detection, "$setOnInsert": {"first_seen": datetime.utcnow()}},
            upsert=True,
        )
        return str(result.upserted_id) if result.upserted_id else "updated"
    ...
    async def deactivate_stale_vehicles(self, camera_id: str, threshold_seconds: int = 10):
        ...
        await self.db.vehicle_detections.update_many(
            {"camera_id": camera_id, "last_seen": {"$lt": threshold}, "active": True},
            {"$set": {"active": False}},
        )
    ...
    async def save_hourly_analytics(self, camera_id: str, analytics: Dict):
        ...
        await self.db.analytics.update_one(
            {"camera_id": camera_id, "date": analytics["date"], "hour": analytics["hour"]},
            {"$set": analytics},
            upsert=True,
        )
    ...
    async def get_analytics_report(self, camera_id: str, days: int = 7) -> List[Dict]:
        ...
        cursor = self.db.analytics.find({"camera_id": camera_id, "date": {"$gte": since}}).sort(
            "date", DESCENDING
        )
        return await cursor.to_list(length=1000)
